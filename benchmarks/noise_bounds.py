...
from benchmarks import noise_intervals
...
def source_identity() -> dict[str, str]:
    return {
        ...
        for path in (Path(__file__), Path(noise_intervals.__file__))
    }


def validate_controls(divisions: tuple[int, ...], max_cells: int, repeats: int) -> None:
    if (
        not divisions
...
def probe(
    ...
    validate_controls(divisions, max_cells, 1)
    ...
    positions = start + np.linspace(0.0, 1.0, REFERENCE_INTERVALS + 1)[:, None] * vector
...
def main() -> None:
    ...
    try:
        validate_controls(tuple(args.divisions), args.max_cells, args.repeats)
